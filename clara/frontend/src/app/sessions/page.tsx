    ...
    <div className={`flex w-full mb-4 ${isClara ? "justify-start" : "justify-end"}`}>
      <div
        className={`max-w-[85%] px-5 py-3.5 rounded-2xl text-[14px] leading-relaxed relative ${
          isClara
            ? "bg-white/[0.04] text-slate-100 rounded-bl-none border border-white/[0.08]"
            : "bg-gradient-to-br from-clara-primary to-clara-primary-light text-white rounded-br-none shadow-glow-sm"
        }`}
      >
        <p className="whitespace-pre-wrap">{message.content}</p>
        <div className={`flex items-center gap-2 mt-2 opacity-50 text-[10px] font-bold uppercase tracking-wider ${isClara ? "text-slate-400" : "text-white"}`}>
...
const SessionCard: React.FC<{ session: SessionHistoryEntry }> = ({ session }) => {
  ...
  return (
    <motion.div 
      ...
      className="glass-card rounded-3xl border-white/[0.05] shadow-xl overflow-hidden mb-3 hover:bg-white/[0.02] transition-colors"
    >
      <button
      ...
      >
        <div className="flex items-center gap-6">
          <div className="shrink-0 w-12 h-12 rounded-2xl bg-white/[0.03] border border-white/[0.08] flex flex-col items-center justify-center">
            <span className="text-xs font-black text-white leading-none mb-0.5">{format(parseISO(session.started_at), "d")}</span>
            <span className="text-[8px] font-black text-clara-text-tertiary uppercase tracking-widest">{format(parseISO(session.started_at), "MMM")}</span>
          </div>

          <div className="flex flex-col">
            <h4 className="text-sm font-black text-white group-hover:text-clara-primary-light transition-all mb-1 tracking-tight">
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-[10px] font-black uppercase tracking-widest text-clara-text-tertiary">
              ...
              {session.alert_count > 0 && <span className="flex items-center gap-1.5 text-danger animate-pulse"><Zap size={10} /> {session.alert_count} Alerts</span>}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          {session.mood_summary && (
            <div className={`hidden md:flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/[0.03] border border-white/[0.08]`}>
      ...
      <AnimatePresence>
        {expanded && (
          <motion.div 
            ...
            className="overflow-hidden border-t border-white/[0.05] bg-white/[0.01]"
          >
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <div className="flex items-center justify-between mb-10">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tighter">Your Sanctuary Wall</h1>
        ...
        <button
          ...
          disabled={loading}
          className="w-12 h-12 glass-card rounded-2xl flex items-center justify-center text-clara-text-tertiary hover:text-white transition-all disabled:opacity-40"
        >
        ...
        <div className="space-y-4">
          ...
          {sessions.length === 0 && !error && (
            <div className="glass-card rounded-[2.5rem] py-32 flex flex-col items-center text-center">
               <div className="w-20 h-20 bg-white/[0.03] rounded-[2.5rem] flex items-center justify-center mb-6 text-clara-primary">
                  <Calendar size={40} />
               </div>
               <h4 className="text-xl font-black text-white mb-2">The wall is empty</h4>
