...
class AlertRepository(BaseRepository[Alert]):
    ...
    async def get_unresolved_alerts(
        ...
        """
        Fetch all unresolved incidents for a specific patient.

        Both patient_id and organization_id are required to enforce multi-tenant
        isolation — prevents cross-tenant data exposure when a patient UUID
        happens to exist in more than one organisation.
        """
    ...
    async def resolve_alert(
        self, alert_id: uuid.UUID, resolved_by: str
    ) -> Optional[Alert]:
    ...
    async def get_org_unresolved_alerts(
